...
# Plans
## `requests`
I already have a working file that inputs a search to scrape and outputs the links to the data associated with that search. This is the lowest level scraping mechanism, along with the scraping of the links it outputs. Function: `get_evaluation_report_links(*attributes)`. I will need to make another function that scrapes the data at a given link to be run on the output of `get_evaluation_report_links`. This should be relatively easy.
scrapeSearch.py:
```
import requests
